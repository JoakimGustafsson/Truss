...
function WalkTruss(view, updatefrequency){
	...
	springconstant = 5000;
	absorbconstant = 50000;
	
	var startTensor = this.addTensor(new Spring(f1, f2, 500));
	...
	this.addTensor(new Spring(f1, b1, springconstant));
	this.addTensor(new Spring(f2, b2, springconstant));
	this.addTensor(new Spring(f3, b3, springconstant));
	...
	var ego = new ProtagonistNode(new Position(2,3), 70,"Ego");
...
//	this.initConnect();
	...
	var actuatorNode = this.addNode(new SpringRunnerNode(ego, new Position(1,0.5), new Position(3,0.5),0.01,"myActuatorNode"));
	
	this.addTensor(new Spring(sensorNode, actuatorNode, 50, 0.5));
}
...
function ProtagonistNode(startPosition, mass=70, name="ProtagonistNode", positionFunction, showFunction, velocityLoss=1){
	Node.call(this, startPosition,mass,name, positionFunction, showFunction, 0.99);
	...
	/*		This should not be needed, instead the Tensor should be called with addNode1(newNode) or addNode2(null)
	ProtagonistNode.prototype.detachTensor = function(){
		this.currentTensor=0;
		this.mySpring.constant=0;
		this.currentTensorOriginalLength = 0;
		this.removeTensor(this.currentTensor);
	}*/
	...
	ProtagonistNode.prototype.attachToTensor = function(tensor,distanceFraction=0.5){
		...
		/*
		if (this.currentTensor.node1.getPosition().x<this.currentTensor.node2.getPosition().x) {
			this.mySpring.node1 = this.currentTensor.node1; 
			this.currentTensor.node1=this;
		}
		else { 
			this.mySpring.node1 = this.currentTensor.node2; 
			this.currentTensor.node2=this;
		}*/
...
/*
	ProtagonistNode.prototype.updateVelocity = function(forceAppliers,timeFactor){	
		Node.prototype.updateVelocity.call(this,forceAppliers,timeFactor);	//Call parent in order to update this nodes position
	}
	*/
